...
extension String {
    ...
    var generatedColor: Color {
        ...
        return .init(red: randomColorComponentValue(using: &randomNumberGenerator),
                     green: randomColorComponentValue(using: &randomNumberGenerator),
                     blue: randomColorComponentValue(using: &randomNumberGenerator))
    }
    
    private func randomColorComponentValue(using generator: inout RandomNumberGeneratorWithSeed,
                                           maximumColorComponentValue: Double = 0.7) -> Double {
        .random(in: 0...maximumColorComponentValue, using: &generator)
    }
